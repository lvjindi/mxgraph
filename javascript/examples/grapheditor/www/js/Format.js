...
TextFormatPanel.prototype.addFont = function (container) {
    ...
    if (graph.getModel().isEdge(cell)) {
        ...
        guardPanel.style.paddingTop = '10px';
        guardPanel.style.paddingBottom = '28px';
        guardPanel.style.fontWeight = 'normal';

        var selectTitle = this.createTitle(mxResources.get('select'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        var guardTitle = this.createTitle(mxResources.get('guard'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        var syncTitle = this.createTitle(mxResources.get('sync'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        var updateTitle = this.createTitle(mxResources.get('update'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        createExtraInput(selectTitle, 'select', container, guardPanel);
        createExtraInput(guardTitle, 'guard', container, guardPanel);
        createExtraInput(syncTitle, 'sync', container, guardPanel);
        ...
        namePanel.style.paddingBottom = '28px';
        namePanel.style.fontWeight = 'normal';

        var nameTitle = this.createTitle(mxResources.get('name'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        var invariantTitle = this.createTitle(mxResources.get('invariant'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        var rateOfExponentialTitle = this.createTitle(mxResources.get('rateOfExponential'));
        title.style.paddingLeft = '5px';
        title.style.paddingTop = '5px';
        title.style.paddingBottom = '4px';

        createExtraInput(nameTitle, 'name', container, namePanel);
        createExtraInput(invariantTitle, 'invariant', container, namePanel);
        createExtraInput(rateOfExponentialTitle, 'rateOfExponential', container, namePanel);

        var selectPanel = namePanel.cloneNode(false);
        ...
        div.style.overflow = 'hidden';
        div.style.width = '200px';
        div.style.height = (mxClient.IS_QUIRKS) ? '27px' : '55px';
        createCheckbox(cell, div, 'initial', container, selectPanel)
    ...
    function createExtraInput(title, label, container, namePanel) {
        ...
        div.style.overflow = 'hidden';
        div.style.width = '200px';
        div.style.height = (mxClient.IS_QUIRKS) ? '27px' : '60px';
        ...
        mxUtils.write(input, mxResources.get(label));
        div.appendChild(title);
        div.appendChild(input)
        namePanel.appendChild(div)
        ...
        if (label == 'name') {
            input.value = cell.getName() != null ? cell.getName() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                graph.getModel().beginUpdate();
                try {
                    graph.getModel().setName(cell, input.value);
                } finally {
                    graph.getModel().endUpdate();
                }
                mxEvent.consume(evt);
            })
        } else if (label == 'invariant') {
            input.value = cell.getInvariant() != null ? cell.getInvariant() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                graph.getModel().beginUpdate();
                try {
                    graph.getModel().setInvariant(cell, input.value);
                } finally {
                    graph.getModel().endUpdate();
                }
                mxEvent.consume(evt);
            })
        } else if (label == 'rateOfExponential') {
            input.value = cell.getRateOfExponential() != null ? cell.getRateOfExponential() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                graph.getModel().beginUpdate();
                try {
            ...
            input.value = cell.getSelect() != null ? cell.getSelect() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                cell.setSelect(input.value);
                edgeStringToValue(cell, input.value);
                mxEvent.consume(evt);
            })

        } else if (label == 'guard') {
            input.value = cell.getGuard() != null ? cell.getGuard() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                cell.setGuard(input.value);
                edgeStringToValue(cell, input.value);
                mxEvent.consume(evt);
            })
        } else if (label == 'sync') {
            input.value = cell.getSync() != null ? cell.getSync() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                cell.setSync(input.value);
                edgeStringToValue(cell, input.value);
                mxEvent.consume(evt);
            })
        } else {
            input.value = cell.getUpdate() != null ? cell.getUpdate() : '';
            mxEvent.addListener(input, 'change', function (evt) {
                cell.setUpdate(input.value);
                edgeStringToValue(cell, input.value);
                mxEvent.consume(evt);
            })
        }
    ...
    function createCheckbox(cell, div, label, container, selectPanel) {
    ...
    function vertexStringToValue(cell, value) {
        if (value != null) {
            var name = (cell.getName() == null) ? '' : cell.getName();
            var invariant = (cell.getInvariant() == null) ? '' : cell.getInvariant();
            // if (invariant.indexOf('<') != -1) {
            //
            //     alert(invariant)
            // }
            var rateOfExpoential = (cell.getRateOfExponential() == null) ? '' : cell.getRateOfExponential();
            var str = '<b><font color="#a52a2a">' + name + '</font></b><br><br>' +
                '<b><font color="#00008b">' + invariant + '</font></b><br><br>' +
                '<b><font color="black">' + rateOfExpoential + '</font></b>';
            graph.getModel().setValue(cell, str);
        }
    }
...
DiagramFormatPanel.prototype.init = function () {
    ...
    if (graph.isEnabled()) {
        ...
        this.container.appendChild(this.addParameter(this.createPanel()));
